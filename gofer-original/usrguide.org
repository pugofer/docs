...
[[*Appendix D: USING GOFER WITH BIRD+WADLER][Appendix D]]. More importantly, the underlying semantics of Gofer do
...
[[*APPENDIX C: RELATIONSHIP WITH HASKELL 1.1][Appendix C]].
...
described in [[*APPENDIX F: INTERPRETER COMMAND SUMMARY][Appendix F]].
...
- An identifier begins with a letter of the alphabet optionally followed
  ...
  [[*Datatype definitions][section 11.1]]). The following identifiers are examples of
...
expressions as described in [[*USER-DEFINED DATATYPES AND TYPE SYNONYMS][section 11]].
...
[[*STANDARD AND USER-DEFINED FUNCTIONS][section 5]]:
...
- Irrefutable patterns :: A further kind of pattern (called an irrefutable pattern)
  is introduced in [[*Pattern bindings and irrefutable patterns][section 9.11]].
...
factorial function from [[*STANDARD AND USER-DEFINED FUNCTIONS][section 5]].
...
[[*Simple pattern matching][[section 9.1]]:
...
arranged in decreasing order, as described in [[*Arithmetic sequences][section 10.1]]).
...
calculation from [[*USING GOFER — A BASIC INTRODUCTION][section 4]] to find the sum of the integers 1 to 10:
...
** Polymorphism
Given the definition of ~product~ in [[*Recursion with lists][section 9.5]], it is easy to see that
...
[[*Simple pattern matching][section 9.1]]. As a simple example of pattern bindings, here is one
...
particularly convenient in some situations (see [[*DIALOGUES: INPUT AND OUTPUT][section 12]] for some
...
- Overloading ::
     ...
     [[*OVERLOADING IN GOFER][section 14]] for further details.
...
in [[*USING GOFER — A BASIC INTRODUCTION][section 4]] used to calculate the sum of the integers from 1 upto 10 --
...
approach to I/O described in [[*DIALOGUES: INPUT AND OUTPUT][section 12]].
...
or: pat | condition1 -> rhs1      using guard expressions as
        | condition2 -> rhs2      described in [[*Guarded equations][section 9.2]] for
...
or, using lambda expressions as introduced in [[*Lambda expressions][section 10.3]]:
...
As described in [[*Type declarations][section 9.12]], it is often useful to be able to declare
...
overloaded functions and values as described in [[*OVERLOADING IN GOFER][section 14]].
...
either using an explicitly typed expression ([[*Explicitly typed expressions][section 10.6]]) or a type
declaration ([[*Type declarations][section 9.12]]):
...
matching as described in [[*MORE ABOUT VALUE DECLARATIONS][section 9]] would force Gofer to try to match the
...
in [[*APPENDIX B: CONTENTS OF STANDARD PRELUDE][appendix B]]:
...
for these functions using the stream based approach from section [[*Basic description][12.1.]]]
...
that will be created from the instance definitions in  [[*Instances of class Eq][section 14.2.2]] for
...
- Given the definition of a non-strict ordering (<=) on the elements of
  ...
  described in [[*Testing equality of represented values][section 14.2.3]], in terms of the subset ordering defined
  in class Ord:
...
functions are defined by variable declarations as in [[*Variable declarations][section 9.10]]. As an
...
of implicitly overloaded numeric constants, described in [[*Overloading and numeric values][section 14.4.4]],
...
|-----------+----------------------------------------------------------------------------|
| ~VARID~   | identifier beginning with lower case letter as described in [[*FUNCTION NAMES — IDENTIFIERS AND OPERATORS][section 6]].     |
| ~CONID~   | like ~VARID~, but beginning with upper case letter.                        |
| ~VAROP~   | operator symbol not beginning with a colon ~:~, as described in [[FUNCTION NAMES — IDENTIFIERS AND OPERATORS][section 6]]. |
...
| ~FLOAT~   | floating point constant, as described in [[*Floating point numbers][section 7.4]].                      |
...
[[*BACKGROUND AND ACKNOWLEDGEMENTS][section 2]].
...
- Instances of type classes can be defined at non-overlapping, but
  otherwise arbitrary types, as described in [[*Equality on function types][section 14.2.5]].
- List comprehensions may include local definitions, specified by
  qualifiers of the form <pat>=<expr> as described in [[*List comprehensions][section 10.2]].
- No restrictions are placed on the form of predicates that appear in
  ...
  available for holding dictionaries (see [[*‘Recursive’ dictionaries][section 14.4.2]]).
** Other differences
...
- The differences between the syntax used for guarded equations in Gofer
  ...
  [[*Guarded equations][section 9.2]]. For example:
...
:UNNUMBERED:
:END:
- <expr>               ::  Analyse expression for errors, typecheck and evaluate. If the expression has type Dialogue, execute as a program using the I/O facilities as described in [[*DIALOGUES: INPUT AND OUTPUT][section 12]]. If the expression has type String, evaluate and print result as a lazy list of characters. In any other case, the standard prelude function show' is applied to the expression and used to print the value of the result in the form of a string, as in the previous case.
